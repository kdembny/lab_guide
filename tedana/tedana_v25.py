...
if __name__=='__main__':
	...
	parser.add_option('-m',"--mask",dest='mask_path',help="Path to pre-computed intracranial mask",default=None)
	...
	if options.mask_path != None:
		mask_nib  = nib.load(options.mask_path)
		mask = mask_nib.get_data()
		mask = mask.astype(np.bool)
		print ("++ Dimensions of User Provided Mask: %d,%d,%d" % (np.shape(mask)))
		type(mask)
	else:
		mask  = makemask(catd)
		print ("++ Dimensions of Newly Created Mask: %d,%d,%d" % np.shape(mask))
		type(mask)
        niwrite(mask,aff,'mask.nii')
