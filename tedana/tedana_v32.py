...
if __name__=='__main__':
	...
	if options.mixm!=None: 
		try: 
			...
			print('++ JAVIER: cp %s %s/meica_mix.1D; cp %s %s/%s' % (options.mixm,dirname,options.mixm,dirname,os.path.basename(options.mixm)))
		except: pass
	if options.ctab!=None: 
		try: 
			os.system('cp %s %s/comp_table.txt; cp %s %s/%s' % (options.ctab,dirname,options.ctab,dirname,os.path.basename(options.ctab)))
			print('++ JAVIER: cp %s %s/compt_table.txt; cp %s %s/%s' % (options.ctab,dirname,options.ctab,dirname,os.path.basename(options.ctab)))
		except: pass
	...
	if options.mixm == None:
		...
		print "++ Using Components provided by user"
		mmix_orig = np.loadtxt('meica_mix.1D')
		print "++ INFO: mmix_orig.shape = %s" % str(mmix_orig.shape)
		print "++ INFO: mask.shape %s" % str(mask.shape)
		print "++ INFO: catd.shape %s" % str(catd.shape)
		eim = eimask(np.float64(fmask(catd,mask)))==1
		print "++ INFO: eim.shape %s" % str(eim.shape)
		...
		if options.ctab == None: 
			print "++ Creating Comp Table"
			acc,rej,midk,empty = selcomps(seldict,knobargs=args,debug=debug_mode,strict_mode=options.strict)
		else: 
			print "++ Using User provided Comp Table"
			#acc,rej,midk,empty = ctabsel(ctabfile)
			acc,rej,midk,empty = ctabsel(options.ctab)
	print "++ INFO: ACC=%s" % str(acc)
	print "++ INFO: REJ=%s" % str(rej)
	print "++ INFO: MIDK=%s" % str(midk)
	print "++ INFO: IGN=%s" % str(empty)
	if len(acc)==0:
